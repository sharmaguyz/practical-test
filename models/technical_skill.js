...
class TechnicalSkill {
    constructor() {
        this.tableName = dynamoDBService.tables.technical_skills.tableName;
        this.tableSchema = dynamoDBService.tables.technical_skills.schema;
    ...
    async checkAndCreateTable() {
        try {
            ...
            if (error.code === 'ResourceNotFoundException') {
                ...
                // Create the table
                await dynamoDBService._createTable('technical_skills', this.tableSchema);
    ...
    async seedTechnicalSkillsData(seedData = []) {  
        if (!seedData.length) {
            console.log('⚠️ No data provided for technical skills seeding.');
            return;
        }
        const params = {
            RequestItems: {
        ...
        try {
            ...
            console.log(`✅ Seeded ${seedData.length} technical skills.`);
        } catch (err) {
            console.error('❌ Failed to seed technical skills:', err);
        }
    }
    ...
    static async findAll() {
        const params = {
            TableName: dynamoDBService.tables.technical_skills.tableName,
        };
