...
class Certification {
    constructor() {
        this.tableName = dynamoDBService.tables.certifications.tableName;
        this.tableSchema = dynamoDBService.tables.certifications.schema;
    ...
    async checkAndCreateTable() {
        try {
            ...
            if (error.code === 'ResourceNotFoundException') {
                ...
                // Create the table
                await dynamoDBService._createTable('certifications', this.tableSchema);
    ...
    async seedCertificationData(seedData = []) {  
        if (!seedData.length) {
            console.log('⚠️ No data provided for certification seeding.');
            return;
        }
        const params = {
            RequestItems: {
        ...
        try {
            ...
            console.log(`✅ Seeded ${seedData.length} certifications.`);
        } catch (err) {
    ...
    static async findAll() {
        const params = {
            TableName: dynamoDBService.tables.certifications.tableName,
        };
        try {
