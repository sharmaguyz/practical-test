...
class SecurityClearanceLevel {
    constructor() {
        this.tableName = dynamoDBService.tables.security_clearance_levels.tableName;
        this.tableSchema = dynamoDBService.tables.security_clearance_levels.schema;
    ...
    async checkAndCreateTable() {
        try {
            ...
            if (error.code === 'ResourceNotFoundException') {
                ...
                // Create the table
                await dynamoDBService._createTable('security_clearance_levels', this.tableSchema);
    ...
    async seedSecurityClearanceLevelsData(seedData = []) {  
        if (!seedData.length) {
            console.log('⚠️ No data provided for security clearance levels seeding.');
            return;
        }
        const params = {
            RequestItems: {
        ...
        try {
            ...
            console.log(`✅ Seeded ${seedData.length} security clearance levels.`);
        } catch (err) {
            console.error('❌ Failed to seed security clearance levels:', err);
        }
    }
    ...
    static async findAll() {
        const params = {
            TableName: dynamoDBService.tables.security_clearance_levels.tableName,
        };
...
module.exports = SecurityClearanceLevel;