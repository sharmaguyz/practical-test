        ...
        },
        {
            ...
            renderCell: (params) => {
                const handleShowDetails = async () => {
                    return router.push(`/instructor/course-invoice/${params.row.orderId}/${params.row.courseId}`);
                }
                const handleDetails = async () => {
                    return router.push(`/instructor/course-details/${params.row.courseId}`);
                }
                return (
                    <>
    ...
    const fetchTableData = useCallback(async () => {
            ...
            try {
                ...
                const response: any = await fetchData(
                    `${API_BASE_URL}/api/v1/instructor/purchased-courses?page=${paginationModel.page + 1}&limit=${paginationModel.pageSize}&search=${searchText}`, token, IdToken, AccessToken
                );
                if (response?.error) {
