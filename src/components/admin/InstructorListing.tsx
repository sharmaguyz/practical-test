...
export default function UserListing() {
  ...
  const fetchTableData = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetchData(`${API_BASE_URL}/api/v1/admin/get-users?role=INSTRUCTOR&page=${paginationModel.page + 1}&limit=${paginationModel.pageSize}&search=${searchText}`, token)
      if (response?.error) {
    ...
    },
    {
      ...
      renderCell: (params) => {
        const handleDelete = async () => {
          const confirmed = await confirm({ message: 'Are you sure you want to delete this instructor?' });
          if (!confirmed) return;

          try {
            setLoading(true);
            const token = localStorage.getItem('token'); // adjust based on your auth flow
            const response = await fetch(`${API_BASE_URL}/api/v1/admin/delete-user?userId=${params.id}&rolename=INSTRUCTOR`, {
              method: "DELETE",
              headers: {
                "Content-Type": "application/json",
                Authorization: `Bearer ${token}`,
        ...
        return (
          <>
            <Tooltip title="Edit">
              <IconButton
                sx={{ color: '#633793' }}
                onClick={() => router.push(`/admin/instructors/add-form?id=${params.row.id}`)}
              >
                <EditIcon />
              </IconButton>
            </Tooltip>
  ...
  return (
    <DataTable
      ...
      is_show_button={true}
      button_text="Add New Instructor"
      button_link="/admin/instructors/add-form"
    />
  );
}
